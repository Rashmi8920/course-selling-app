...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response) {
        // setErrorMessage(error.response.data.ehrrors || "Login failed!!!");
         toast.error("error in login" || error.response.data.errors)
    ...
    <div className="bg-gradient-to-r from-black to-blue-950 ">
      <div className="h-screen container mx-auto flex  items-center justify-center text-white">
        ...
        <header className="absolute top-0 left-0 w-full flex justify-between items-center p-5  ">
          ...
          <div className="flex items-center sm:ml-0 mr-[-18px] sm:mr-15 sm:gap-2">
            <Link
              to={"/signup"}
            ...
            <Link
              to={"/courses"}
              className="bg-orange-500 p-1  text-sm md:text-md md:py-2 md:px-4 rounded-md ml-2"
            >
              Join now
            </Link>
          </div>
