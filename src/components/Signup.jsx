...
function Signup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();   //after submit browder  refresh na ho
 ...
 navigate("/login");
      ...
      if (error.response) {
        alert(error.response.data.errors)
         toast.error("error in signup" || error.response.data.errors)
     }
    }
  };
  ...
  return (
    <div className="bg-gradient-to-r from-black to-blue-950 ">
      <div className="sm:h-screen container mx-auto flex  items-center justify-center text-white">
        {/* Header */}
        <header className="absolute top-0 left-0 w-full flex justify-between items-center p-5  ">
          <div className="flex items-center space-x-1">
            <img src={logo} alt="Logo" className="w-10 h-10 rounded-full" />
            <Link to={"/"} className="sm:text-2xl text-xl font-bold text-orange-500">
              CourseHaven
            </Link>
          </div>
          <div className="flex items-center gap-2 sm:mr-15 sm:gap-2 mr-[-15px] ">
            <Link
              to={"/login"}
        ...
        {/* Signup Form */}
        <div className="bg-gray-900 p-8 rounded-lg shadow-lg w-[500px] m-8 md:m-0 sm:mt-40 mt-30">
          ...
          <form onSubmit={handleSubmit} >
            ...
            <div className="mb-4">
              <label htmlFor="password" type="password" className=" text-gray-400 mb-2">
                Password
              </label>
      ...
      >
        {showPassword ? "🙈" : "👁️"}  {/* 👈 icon bhi toggle */}
      </span>
              </div>
            </div>
            {errorMessage && (
