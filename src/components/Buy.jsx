...
useEffect(() => {
  const fetchCourseDetails = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/course/create-payment-intent/${courseId}`,
        {},
        {
          headers: {
      ...
      if (error?.response?.status === 400) {
        ...
        toast.error(error?.response?.data?.errors || "Error loading course");
      } } };
  ...
  const handlePurchase = async (event) => {
    ...
    if (card == null) {
      console.log("card elemenet not found")
      setLoading(false)
      return;
    }
    ...
    if (confirmError) {
      ...
      console.log("payment succedded", paymentIntent)
      ...
      await axios.post(`${import.meta.env.VITE_BASE_URL}/api/v1/order`, paymentInfo, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        withCredentials: true,
...
// 1 problem course page m orahnecolor buy btn per cloick krtai hi course automatrically purchased ho jara 
