...
function Dashboard() {
  const handleLogout = async () => {
    try {
      ...
      toast.error(error.response.data.errors || "Error in admin logout");
    }
  };
  return (
    <div className="flex h-screen">
      {/* Sidebar */}
      <div className="w-64 bg-gray-100 sm:p-5 p-3">
        ...
        <nav className="flex flex-col space-y-4">
          ...
          <Link to="/admin/login">
           ...
           <Link to="/admin/orders">
            <button className="w-full bg-blue-600  hover:bg-sky-500 text-white py-2 rounded">
            All Orders
            </button>
          </Link>
        </nav>
      </div>
      <div className="flex h-screen items-center sm:text-3xl text-xl font-semibold justify-center sm:ml-[30%]">
        Welcome!!!
      </div>
    </div>
  );
